...
abstract class CalendarPagerView extends ViewGroup implements View.OnClickListener {
    ...
    public CalendarPagerView(@NonNull MaterialCalendarView view,
        ...
        buildWeekDays(resetAndGetWorkingCalendar());
        buildDayViews(dayViews, resetAndGetWorkingCalendar(), type);
    }
    ...
    protected void buildDayViews(Collection<DayView> dayViews, Calendar calendar, int type) {
    }
    ...
    public void setWeekDayTextAppearance(int taId) {
        for (WeekDayView weekDayView : weekDayViews) {
            ...
            if (mcv.getType() == 1) {
                weekDayView.setTextColor(getContext().getResources().getColor(R.color.gray001));
            }
        }
    }
        ...
        for (DayView dayView : dayViews) {
            ...
            if (mcv.getType() == 1) {
                ...
                if (day.isBefore(currentDay)) {
                    dayView.setTextColor(getContext().getResources().getColor(R.color.gray001));
                }
            }
        }
    }
    ...
    public void setSelectionEnabled(boolean selectionEnabled) {
        for (DayView dayView : dayViews) {
            CalendarDay day = dayView.getDate();
            if (day.isBefore(currentDay) && mcv.getType() == 1) {
...
//        final int measureTileHeight = (int) (measureTileWidth * 0.66);
