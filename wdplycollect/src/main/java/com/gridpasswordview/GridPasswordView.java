...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        textColor = ta.getColorStateList(R.styleable.gridPasswordView_textColor_gridPasswordView);
        if (textColor == null)
            textColor = ColorStateList.valueOf(getResources().getColor(android.R.color.black));
        int textSize = ta.getDimensionPixelSize(R.styleable.gridPasswordView_textSize_gridPasswordView, -1);
        if (textSize != -1) {
            this.textSize = Util.px2sp(context, textSize);
        }

        lineWidth = (int) ta.getDimension(R.styleable.gridPasswordView_lineWidth_gridPasswordView, Util.dp2px(getContext(), 1));
